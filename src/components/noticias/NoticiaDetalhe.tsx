...
export function NoticiaDetalhe({
  ...
  return (
    <article className="mx-auto flex max-w-6xl flex-col items-center px-3 pt-6 text-center sm:px-6 sm:pt-10 lg:pt-12">
      {/* Kicker — Instrument Serif */}
      <p className="serif uppercase text-[clamp(0.9rem,3vw,1.35rem)] text-text">
        {kicker}
      </p>
      ...
      <p className="mt-8 max-w-4xl font-display italic text-[clamp(0.95rem,3vw,1.6rem)] leading-[1.45] text-text sm:mt-12 lg:mt-15">
        {subtitle}
      </p>

      {/* Publicado — Instrument Serif */}
      <p className="serif mt-8 uppercase tracking-[0.2em] text-[clamp(0.72rem,1.4vw,0.85rem)] text-text-muted sm:mt-12 sm:tracking-[0.25em] lg:mt-15">
        {publishedAt}
      </p>

      {/* Corpo — Poppins, com capitular de 3 linhas */}
      <div className="mt-8 w-full max-w-5xl text-left sm:mt-12 lg:mt-15">
      ...
      {quote && (
        <blockquote className="mt-10 max-w-3xl px-2 font-display italic text-[clamp(0.95rem,2.6vw,1.4rem)] leading-[1.6] text-text sm:mt-14 sm:px-0">
          {`“${quote}”`}
        </blockquote>
      )}

      {/* Imagem full-width (sangra de ponta a ponta da tela) */}
      {image && (
        <div className="relative left-1/2 mt-10 w-screen max-w-none -translate-x-1/2 self-start sm:mt-14">
          <Image
            src={image}
            alt={imageAlt}
            sizes="100vw"
      ...
      {bodyClosing && (
        <div className="mt-6 w-full max-w-5xl text-left sm:mt-8">
